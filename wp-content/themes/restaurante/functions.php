...
function ketchupthemes_widgets_init() {
    ...
    register_sidebar(array(
        ...
        'id'   => 'sidebar',
        'description' => __('This is the widgetized sidebar.', 'restaurante' ),
      ...
      register_sidebar(array(
        'name' => __('Footer Sidebar Left Column', 'restaurante' ),
     ...
     register_sidebar(array(
        'name' => __('Footer Sidebar Right Column', 'restaurante' ),
...
function ketchupthemes_wp_title($title,$sep){
    ...
    if ( $paged >= 2 || $page >= 2 )
        ...
        $title = "$title $sep " . sprintf( __( 'Page %s', 'restaurante' ), max( $paged, $page ) );
        return $title;
}
add_filter( 'wp_title', 'ketchupthemes_wp_title', 10, 2 );
add_filter('mce_buttons','ketchupthemes_wysiwyg_editor');

function ketchupthemes_wysiwyg_editor($mce_buttons) {
...
wp_nonce_field( 'ketchupthemes_pricefield_inner_custom_box', 'ketchupthemes_pricefield_inner_custom_box_nonce' );
  ...
  $value = get_post_meta( $post->ID, 'kt_price', true );

  echo '<label for="pricefield_new_field">';
       _e( "Add a price", 'restaurante' );
  echo '</label> ';
...
function ketchupthemes_pricefield_save_postdata( $post_id ) {
  ...
  update_post_meta( $post_id, 'kt_price', $mydata );
}
    ...
    function get_settings_sections() {
        $sections = array(
            array(
                ...
                <p>Visit this link here to know more.<p><a class="premium_link" href="'.esc_url('http://ketchupthemes.com/restaurante-theme','restaurante').'">Restaurante Theme- Premium Edition</a></p>
                </div>')
    ...
    function get_settings_fields() {
        $settings_fields = array(
            THEMEPREFIX.'_general' => array(
             array(
                    'name' => 'favicon',
                    'label' => __( 'Upload Favicon', 'restaurante' ),
                    'desc' => __( '<p>Upload your favicon.Make sure it is 16x16pixels.</p>', 'restaurante' ),
                    'type' => 'file',
                    'default' => '',
